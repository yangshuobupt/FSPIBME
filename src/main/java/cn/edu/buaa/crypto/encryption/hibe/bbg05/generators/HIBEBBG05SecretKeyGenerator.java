...
public class HIBEBBG05SecretKeyGenerator implements PairingKeyParameterGenerator {
    ...
    public FSPIBMEESKSerParameter generateEKey() {
        if (params instanceof HIBESecretKeyGenerationParameter) {
            ...
            HIBEBBG05SecretKeySerParameter X_rho = new HIBEBBG05SecretKeySerParameter(publicKeyParameters.getParameters(),
                    parameters.getIds(), elementIds, a0, a1, hs);
            Element R_rho = publicKeyParameters.getG().powZn(r).getImmutable();
            return new FSPIBMEESKSerParameter(X_rho, R_rho);
