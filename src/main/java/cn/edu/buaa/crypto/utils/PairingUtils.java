...
//    }
    ...
    //判断a是不是b的祖先节点
    public static boolean IsFather(String a, String b) {
        if (a == "E" && b != "E")
            return true;
    ...
    public static String GetFather(String a) {
        if (a == "E")
            return null;
        if (a == "0" || a == "1")
            return "E";
    ...
    public static String GetSibling(String a) {
        if (a == "E")
            return null;
        if (a == "0") return "1";
        if (a == "1") return "0";
    ...
    public static Map<String, FSPIBMEESKSerParameter> PunctureTree(PairingKeySerParameter publicKeyParameter, HIBEBBG05Engine hibebbg05Engine, Map<String, FSPIBMEESKSerParameter> tk, String n) {

        Map<String, FSPIBMEESKSerParameter> tkP = new HashMap<String, FSPIBMEESKSerParameter>();
        ...
        while (it.hasNext()) {
            String nP = it.next();
            if (!IsFather(nP, n) && !nP.equals(n) && !IsFather(n, nP)) {
                tkP.put(nP, tk.get(nP));
            } else {
                ntmp = n;
                while (GetFather(ntmp) != null && nP.length() < ntmp.length()) {

                    String nodeString = GetSibling(ntmp);
                    if (IsFather(nP, nodeString)) {
                        String delegateString = null;
                        for (int i = nP.length(); i < nodeString.length(); i++) {
                            if (delegateString == null)
                                delegateString = String.valueOf(nodeString.charAt(i));
                            else {
                                String tempString = String.valueOf(nodeString.charAt(i));
                                delegateString = delegateString + tempString;
                            }
                        }
                        if (!tkP.containsKey(nodeString)) {
                            FSPIBMEESKSerParameter delegateKey = hibebbg05Engine.delegateESK(publicKeyParameter, tk.get(nP).getX_rho(), delegateString);
                            tkP.put(nodeString, delegateKey);
                        }
    ...
    public static void setCompare(Set<String> set1, Set<String> set2) {
        ...
        System.out.println("个数 " + result.size() + " 交集：" + result);
        ...
        System.out.println("个数 " + result.size() + " 差集：" + result);
//
//        result.clear();
//        result.addAll(set1);
//        result.addAll(set2);
//        System.out.println("并集：" + result);
    ...
    public static byte[] twoByteXor(byte[] b1, byte[] b2) {
